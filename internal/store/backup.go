...
// ListAllPhotos returns every photo row of one user, id order, for backup.
func (s *Store) ListAllPhotos(ctx context.Context, userID string) ([]PhotoRow, error) {
	rows, err := s.pool.Query(ctx, `SELECT `+photoCols+` FROM photos WHERE user_id = $1 ORDER BY id`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) RestoreDecision(ctx context.Context, userID string, d DecisionRow) (int64, bool, error) {
	var id int64
	err := s.pool.QueryRow(ctx, `INSERT INTO decisions (user_id, action, name, anchor_span_start, anchor_span_end,
		...
		RETURNING id`,
		userID, d.Action, d.Name, d.AnchorStart, d.AnchorEnd, d.AnchorDest.Lat, d.AnchorDest.Lon,
	...
	err = s.pool.QueryRow(ctx, `SELECT id FROM decisions
		WHERE user_id = $5 AND anchor_span_start = $1 AND anchor_span_end = $2
		  AND anchor_dest_lat = $3 AND anchor_dest_lon = $4`,
		d.AnchorStart, d.AnchorEnd, d.AnchorDest.Lat, d.AnchorDest.Lon, userID).Scan(&id)
	return id, false, err
}
...
// idempotent).
func (s *Store) RestorePhoto(ctx context.Context, userID string, p PhotoRow) (bool, error) {
	ct, err := s.pool.Exec(ctx, `INSERT INTO photos (user_id, decision_id, content_hash, original_name, taken_at,
